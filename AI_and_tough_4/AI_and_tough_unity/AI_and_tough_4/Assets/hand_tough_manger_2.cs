...
public class hand_tough_manger_2 : MonoBehaviour {
            ...
            else if (Input.touches[0].phase == TouchPhase.Moved)//移動
            {
                ...
                else if (move_point == false)
                {
                    ...
                    else if (Mathf.Abs(one_finger_displacement.x) < Mathf.Abs(one_finger_displacement.y))
                    {
                        ...
                        if (point.transform.rotation.eulerAngles.x < max_angle)
                        {
                            //當X軸卡到90度時停止旋轉
                            if (point.transform.rotation.eulerAngles.x + one_finger_displacement.y * rotate_speed * Time.deltaTime > max_angle)
                            {
                                return;
                            }
                            else if (point.transform.rotation.eulerAngles.x + (one_finger_displacement.y * rotate_speed * Time.deltaTime) < min_angle)
                            {
                                return;
                            }

                        }
                        //else if (point.transform.rotation.eulerAngles.x > min_angle)
                        //{
                        //    //當X軸卡到-90度時停止旋轉
                        ...
                        //    if ((point.transform.rotation.eulerAngles.x - 360f) + one_finger_displacement.y * rotate_speed * Time.deltaTime < min_angle)
                        //    {
                        //        return;
                        //    }
                        //}
