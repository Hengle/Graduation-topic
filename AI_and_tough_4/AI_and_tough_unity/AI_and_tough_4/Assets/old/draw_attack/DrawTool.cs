...
public class DrawTool : MonoBehaviour {
    ...
    //绘制空心长方形  
    public static void DrawRectangle(Transform t, Vector3 center, float length, float width)
    {
        LineRenderer lr = GetLineRenderer(t);
        lr.SetVertexCount(5);

        lr.SetPosition(0, center - t.right * (width / 2));
        lr.SetPosition(1, center - t.right * (width / 2) + t.forward * length);
        lr.SetPosition(2, center + t.right * (width / 2) + t.forward * length);
        lr.SetPosition(3, center + t.right * (width / 2));
        lr.SetPosition(4, center - t.right * (width / 2));
    }
    ...
    //绘制实心长方形  
    public static void DrawRectangleSolid(Transform t, Vector3 center, float length, float width)
    {
        List<Vector3> vertices = new List<Vector3>();

        vertices.Add(center - t.right * (width / 2));
        vertices.Add(center - t.right * (width / 2) + t.forward * length);
        vertices.Add(center + t.right * (width / 2) + t.forward * length);
        vertices.Add(center + t.right * (width / 2));
